        ...
        if len(dim2) >= 10 and '/' in dim2:
            ...
            if state == 'ROL':
                if cost_code.startswith('98'):
                    # 98-xx recovery/overhead codes: trust the raw dim2 cost type directly
                    col_f = cost_type
                    ...
                    col_f = 'RC'
                # D-job override -> LB, but NOT for 98-xx recovery/overhead codes
                if job_code and job_code[0] == 'D' and not cost_code.startswith('98'):
                    col_f = 'LB'
            # R-job -> CA only outside ROL state; inside ROL the raw cost type (LB) is correct
